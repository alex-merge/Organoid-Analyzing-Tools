...
class OAT():
    ...
    def clusteringEngine(df, center, cIter = 100, cSample = 10, 
        ...
        centroids = [OAT.getCentroid(df.sample(cSample, axis=0)) \
        ...
        selected = OAT.selectCluster(pd.concat([df, cluster], axis = 1),
    ...
    def computeClusters(self, df = "spots", eps = 40, min_samples = 3, 
    ...
    def cleanImage(self):
        ...
        """
        clock = time.time()
        print("# Producing a cleaned, denoised, timelapse ...")
    ...
    def computeSpotsLinks(self):
        ...
        """
        clock = time.time()
        print("# Computing links between spots ...")
    ...
    def getVectors(self, filtering = False, reimport = False, aligned = False,
        ...
        if filtering and "F_SELECT" not in self.tracks.columns:
            self.tracks = clustering.computeClusters(self.tracks, [0, 0, 0])
    ...
    def computeAngularVelocity(self):
        ...
        if not hasattr(self, "tracks") :
            self.getVectors()
        if not "Aligned_X" in self.tracks.columns :
            self.alignRotAxis()
        ...
        for ID in subdf.index:
            ...
            RAvect = self.data.loc[subdf.loc[ID, "TP"], ["Aligned_RA_uX",
                                                         "Aligned_RA_vY",
                                                         "Aligned_RA_wZ"]]
            ...
            velocity = abs((OAT.crossProduct(vectors)**2).sum())**(1/2)/(
                distance[ID])
        ...
        tempdf = pd.DataFrame(columns = ["Mean_AV", "Std_AV"],
                              index = self.data.index, dtype = "float")
        
        for tp in tempdf.index:
            subdf = self.tracks[self.tracks["TP"] == tp].copy()
            mean = subdf["Angular_Velocity"].mean()
            std = subdf["Angular_Velocity"].std()
            
            tempdf.loc[tp] = [mean, std]
        
        self.data = pd.concat([self.data, tempdf], axis = 1)
    ...
    def showAngularVelocity(self, TP, show = True, save = False):
        """
        ...
        if not hasattr(self, "tracks") :
            self.getVectors()
        if not "Angular_Velocity" in self.tracks.columns :
        ...
        # If all the time points then violin plot.
            ...
            sns.violinplot(ax = axs,
                           x = self.tracks["TP"].astype("int"), 
                           y = self.tracks["Angular_Velocity"])
            
        # Showing, saving and closing the figure
        if save :
            if TP == "all":
                savepath = self.dir["avFigs"]+"\\Ang_VelxTime.png"
            else :
                savepath = self.dir["avFigs"]+"\\Ang_Vel_TP"+str(TP)+".png"
    ...
    def showOrgVolume(self, show = True, save = False):
        
        clock = time.time()
        
        if not hasattr(self, "data") or not "volume" in self.data.columns:
